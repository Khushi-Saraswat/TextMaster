...
export default function TextForm(props){
    ...
    const handleSentenceCase = () =>{
      ...
      setText(text.charAt(0).toUpperCase()+text.substring(1));
    ...
    const handleTitleCase = () =>{
          ...
          const captilizedWord = newText.map(
            word => {
                  if (word.length === 0) {
                  return word;
                  }
    ...
    return(
        ...
        <button className="btn btn-primary mx-2 my-3" onClick={handleExtraSpaces}>Remove extra spaces</button>
         <button className="btn btn-primary mx-2 my-3" onClick={handleSentenceCase}>Sentence Case</button>
         <button className="btn btn-primary mx-2 my-3" onClick={handleTitleCase}>Title Case</button>
